...
Module decomposition is a set of submodules that adhere's to the principle of  "information hiding." This principle of information hiding will be used as a fundamental concept for Pong Invader's object oriented development. Even though Pong Invader's is a stand alone project without any forseeable scalability, it will still be extremely beneficial to acheive modular design through module decomposition as Pong Invader's is created in phases.
...
\item[Services:] This module handles outputing the game state produced by all the following sub modules. The submodules are all subject to change if there are changes to the software requirements specifiications.
\item[Implemented By:] GameObject, Bullet, Alien, Player
\end{description}

\subsubsection{GameObjectt}
\begin{description}
...
\item[Services:] This module handles all logic of the Pong Invader's system. All processed logic and mathamatical representations of the game state are then passed to GameObject. 
